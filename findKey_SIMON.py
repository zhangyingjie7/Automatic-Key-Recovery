...
class SIMON_findKey():
    ...
    def genConstraintsBegin(self,r):
        ...
        XL2 = Basics.leftCyclicRotation(XL_copy[0],2)
        XL1 = Basics.leftCyclicRotation(XL_copy[1], 1)
        XL8 = Basics.leftCyclicRotation(XL_copy[2], 8)
...
#         K = self.genVars_roundKey(r)
        ...
        for i in range(self.N):
            # AND
            constraints = constraints + [XL1[i]+' - '+A[i]+ ' >= 0']
            constraints = constraints + [XL8[i]+' - '+A[i]+ ' >= 0']
            ...
            constraints = constraints + [XL2[i]+' - '+YL[i]+ ' >= 0']
    ...
    def genConstraintsEnd(self,r):
        ...
        YR2 = Basics.leftCyclicRotation(YR_copy[0], 2)
        YR1 = Basics.leftCyclicRotation(YR_copy[1], 1)
        YR8 = Basics.leftCyclicRotation(YR_copy[2], 8)
#         K = self.genVars_roundKey(r)
        K1 = self.genVars_roundKey(r+1)
        A = self.genVars_afterAnd_of_round(r)
        ...
        for i in range(self.N):
            ...
            # AND
            constraints = constraints + [YR1[i]+' - '+A[i]+ ' >= 0']
            constraints = constraints + [YR8[i]+' - '+A[i]+ ' >= 0']
            
            # XOR
            constraints = constraints + [A[i]+' - '+XR[i]+ ' >= 0']
            constraints = constraints + [YR2[i]+' - '+XR[i]+ ' >= 0']
    ...
    def countkeyBits(self,f,r1,r,r2):
        ...
        for i in range(r1+r+r2+1-2,r1+r+r2+1):
            ...
            for j in range(self.N):
                if (xPattern[j] == '1'):
                    k3=k3+1
        print("The number of guessed subkeys:")
